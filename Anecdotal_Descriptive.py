"""
Testing out the presence of Anecdotal fallacy in the first 100 tweets using a descriptive prompt
from HugChat API. 
- No context is provided to the model. 
- changes in prompt were made to include examples of the fallacy provided by Jacob. 
"""
...
output_file = "Anecdotal_Descriptive_results.csv"
...
def process_tweet(tweet, idx, retries=3):
    for attempt in range(retries):
        try:
            prompt = f"""You are a critical thinking expert trained to detect logical fallacies in tweets.

Specifically, determine whether the tweet commits an **anecdotal fallacy** — where a speaker uses personal experience or isolated examples instead of valid arguments or evidence to counter general claims.

An anecdotal fallacy often sounds like:
- “I know someone who…”
- “In my case…”
- “This isn’t true for me…”
It is fallacious because one personal case cannot disprove general trends.

### Example (Anecdotal Fallacy):
**Tweet:** "This is sad. I have many friends and family with whom I disagree politically. Refusing to engage outside of your bubble is neither healthy nor unifying."
**Explanation:** This tweet uses personal experience (having politically diverse friends) to dismiss broader concerns about ideological bubbles. It commits an anecdotal fallacy by assuming the speaker’s personal case disproves systemic trends.
...
try:
    ...
    for idx in tqdm(range(len(tweets)), desc="Processing Tweets"):
        ...
        pd.DataFrame(fallacy_results, columns=["Tweet", "Descriptive Results"]).to_csv(output_file, index=False)
