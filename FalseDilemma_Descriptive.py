"""
Testing out the presence of False Dilemma in the first 100 tweets using a descriptive prompt
...
output_file = "FalseDilemma_Descriptive_results.csv"
...
def process_tweet(tweet, idx, retries=3):
    for attempt in range(retries):
        try:
            prompt = f"""You're an expert in logical reasoning. Does this tweet present only two options when more could be present? Answer Yes/No with a brief explanation.
Tweet: "{tweet}"
Results:"""
