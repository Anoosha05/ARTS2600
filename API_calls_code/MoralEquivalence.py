"""
Testing the presence of Moral Equivalence in the tweets using a few-shot prompting approach.
- no context provided to the model
"""
...
output_file = "MoralEquivalence_results.csv"
...
def process_tweet(tweet, idx, retries=3):
    for attempt in range(retries):
        try:
            prompt = f"""
    You are a critical thinking expert trained to identify logical fallacies in short messages and tweets.

    Your task is to determine whether the following tweet commits the Moral Equivalence Fallacy — 
    which occurs when a person falsely compares two actions or situations as being morally equal, 
    even when the ethical contexts or consequences are vastly different.

    This fallacy often sounds like:
    - “They did it too, so it’s the same.”
    - “What they did is just as bad as what we did.”
    - “Both sides are equally wrong.”

    ### Example of a Moral Equivalence Fallacy:
    Tweet: "Sure, we invaded their country, but they’ve hacked our emails — both are equally wrong."
    Explanation: This tweet equates a military invasion with cyber-hacking, ignoring the vastly different scales and consequences.

    ### Example of NOT a Moral Equivalence Fallacy:
    Tweet: "Both sides in this debate have made misleading statements."
    Explanation: The tweet points to a similarity in behavior without equating the moral weight of the actions.

    Now analyze the following tweet:
    Tweet: "{tweet}"

    Does it commit the Moral Equivalence Fallacy? If yes, explain why. If not, explain why not.
Results:"""
